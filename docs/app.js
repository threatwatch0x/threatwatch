/* threatwatch — dashboard logic v2
 * IOC lookup, ATT&CK heatmap, filterbare feed
 */
...
function setupLookup() {
  ...
  function doLookup() {
    ...
    if (matches.length === 0) {
      result.innerHTML = `
        <span class="lookup-clean">
          ✓ Niet gevonden in huidige feed — geen bekende match in ThreatFox, URLhaus of MalwareBazaar.
        </span>
        <span class="lookup-note">Let op: de feed bevat alleen de laatste 24u. Oudere IOC's kunnen verlopen zijn.</span>`;
      return;
    }
    ...
    const rows = matches.slice(0, 5).map(item => `
      <div class="lookup-hit">
        ...
        <span class="lookup-hit-ioc">${escapeHtml(item.ioc)}</span>
    ...
    result.innerHTML = `
      <div class="lookup-found">
        ...
        ${rows}
        ${matches.length > 5 ? `<p class="lookup-more">+ ${matches.length - 5} meer — gebruik de zoekbalk in de feed hieronder.</p>` : ""}
      </div>`;

    // Scroll naar feed en highlight
    state.query = raw;
...
// Mapping van abuse.ch threat_type naar ATT&CK tactic label
...
function getFilteredItems() {
  ...
  return state.items.filter(item => {
    ...
    if (!q) return true;
    return [item.malware, item.ioc, item.threat_type, item.threat_type_description, ...(item.tags || [])]
...
function renderTable() {
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = `<tr class="empty-row"><td colspan="7">geen indicators gevonden — pas je filter aan</td></tr>`;
    updateFootnote(0);
    return;
  }
...
function showError() {
  document.getElementById("feed-body").innerHTML = `
    <tr class="empty-row">
      <td colspan="7">kon de feed niet laden. Is data/feed.json al gegenereerd? Draai scripts/fetch_feeds.py of wacht op de eerste GitHub Actions run.</td>
    </tr>`;
}
...
// Herlaad elke 2 minuten voor het live-gevoel
